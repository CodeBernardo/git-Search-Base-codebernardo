export const renderUserCard = () => {
  const userData = JSON.parse(
    localStorage.getItem("@gitSearch:githubUserInfo")
  );
...
export const renderRepos = () => {
  ...
  userData.repos.forEach((repo) => {
    ...
    if(repo.description === null) {
      repoDescription.innerText = "Este repositório não possui uma descrição..." 
      repoDescription.classList.add('no-description')
    } else {
